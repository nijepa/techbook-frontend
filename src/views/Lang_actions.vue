      ...
      <form class="go-bottom user_form" action="">
        ...
        <div class="form__item">
          ...
          <div class="art__desc">
            ...
            <QuillEditor
              theme="snow"
              v-model:content="language.description"
              contentType="html"
              toolbar="full"
            />
          </div>
...
export default {
  ...
  setup() {
    ...
    const selectedGroup = (group) => {
      return language.value.groups.includes(group);
    };

    const updateGroup = (group) => {
      const groupIdx = language.value.groups.indexOf(group);
      if (groupIdx >= 0) {
        language.value.groups.splice(groupIdx, 1);
      } else {
        language.value.groups.push(group);
      }
    };
    ...
    const saveArticle = () => {
      language.value.langId = lang.value._id;
      if (!edited) {
        store.dispatch("languageAdd", language.value);
      } else {
        store.dispatch("languageUpdate", language.value);
      }
      router.push("/main");
    };
    ...
    const saveLink = (links) => {
      ...
      if (linkIdx || linkIdx === 0) {
        language.value.links[linkIdx] = link.value;
      } else {
        language.value.links.push(links);
      }
      link.value = null;
      linkIdx = null;
    };
    ...
    const updateLink = (links) => {
      link.value = links;
      linkIdx = language.value.links.indexOf(links);
    };

    const removeLink = (links) => {
      language.value.links = language.value.links.filter((a) => a !== links);
    };
